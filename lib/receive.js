...
var FlingReceiver = function ( config ) {
	...
	if ( typeof this._config.scribe === 'function' ) {
		this._scribe = this._config.scribe;
	} else {
		this._scribe = function () {
			// NO-OP
		};
	}
	this._moduleCache = {};
...
FlingReceiver.prototype._handleRequest = function ( transport, rpc ) {
	...
	var tasks = {
		...
		startLogging:           function ( done ) {
			...
			self._scribe( 'info', 'RPC request: ' + rpc.requestNumber + '', payload );

			rpc.response.once( 'send', function ( response ) {

				var stop = new Date().getTime();

				self._scribe( 'info', 'RPC response: ' + rpc.requestNumber + ': ' + (stop - start) + 'ms', response );
