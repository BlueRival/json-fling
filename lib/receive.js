		...
		executeRequest:        function ( done ) {
			if ( typeof method.module[method.action] === 'function' ) {
				try {
					...
					var params = self._config.authorize ? { agent: request.agent, params: request.payload.params } : request.payload.params;
...
FlingReceiver.prototype._authorize = function ( request, done ) {
	if ( this._config.authorize ) {
		try {
			this._config.authorize( { agent: request.agent, payload: request.payload }, function ( authorized ) {
